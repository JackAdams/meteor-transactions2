...
Transact.prototype.commit = function(txid,callback,newId) {
  ...
  if (_.isEmpty(this._items)) {
    ...
    try {
      ...
      Meteor.call("_meteorTransactionsProcess",this._transaction_id, this._description, this._items, this.context, function (err, res) {
        if (err) {
          console.log(err);    
        }
        else {
...
// All processing of a queue of items that writes to the db should go through this function
// The only exceptions are instant actions

Transact.prototype._repairIncomplete = function (items, type) {
  // items is an array of actions
  // type is a string -- possible values "commit", "rollback", "undo", "redo", "finishIncomplete", "rollbackIncomplete"
  // ROLLBACK SPEC
